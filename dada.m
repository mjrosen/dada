...
%main loop of updating T and clustering begins here
while ~ismember(ERRa(:)',prevERR,'rows') && (~noUpdate || iteration==1) ...
        && ~( iteration > 3 && sum(sum(ERRa-diag(diag(ERRa)))) > ...
        sum(sum(reshape(prevERR(iteration-1,:),4,4) - ...
        diag(diag(reshape(prevERR(iteration-1,:),4,4))))))
    %termination conditions:
    %1. we have already clustered with the current value of T
    %2. we were told never to update T after the first iteration
    %3. to prevent chaotic behavior of T for very deep sets, stop if the 
    %sum of the (context-indep) error probabilities increased on the last 
    %round but only after a 3-round burn-in    
    ...
    function pUpdate()
        for b = union(templateUpdated,union(RUpdated,elsUpdated))
            ...
            while(1)
                ...
                %interpolate the log10 of the p-value for the observed lambda
                %and transform pack to no log10
                ...
                if p * length(bin) < omegaR
                    ...
                    bin(b).pS = true;
                    disp 'a significant singleton pval';
                    break;
                elseif lambda < E.Lmaxdmax && E.dmax < 10
...
for i = 1:length(bin)
    ...
    if maxr < LOW_ABUNDANCE_THRESHOLD
        raws = [bin(i).fam.raw];
        reals{i} = int2nt(consensus(scoreMat,{raws.seq},[raws.reads]));
    else
